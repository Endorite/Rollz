...
import com.endorite.rollz.LevelCompleteWindow;
import com.endorite.rollz.LevelCompleteWindow.StarsCount;
import com.endorite.rollz.Player;
...
public class GameScene extends BaseScene implements IOnSceneTouchListener{
	...
	private void generateMap(){
		for (int y = -8; y < 8; y++){
			for (int x = -8; x < 8; x++){
				attachChild(new Sprite(x*15, y*15, resourcesManager.tile_grass_region, vbom));
			}
		}
        camera.setBounds(-8*16, -8*16, 8*16, 8*16); // here we set camera bounds
	...
	private void loadLevel(int levelID)
	{
	    ...
	    levelLoader.registerEntityLoader(new EntityLoader<SimpleLevelEntityLoaderData>(TAG_ENTITY)
	    {
	        public IEntity onLoadEntity(final String pEntityName, final IEntity pParent, final Attributes pAttributes, final SimpleLevelEntityLoaderData pSimpleLevelEntityLoaderData) throws IOException
	        {
	            ...
	            {
	                levelObject = new Sprite(x*16, y*16, resourcesManager.tile_grass_region, vbom);
